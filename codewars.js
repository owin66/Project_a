...
function toAlternatingCase(str) {
  return str
    .split("")
    .map((a, b) => (a == a.toUpperCase() ? a.toLowerCase() : a.toUpperCase()))
    .join("");
}
...
function formatMessage(message, maxLength) {
  ...
  result =
    message.length <= maxLength
      ? message.slice(message.length[0], maxLength)
      : message.slice(message.length[0], maxLength) + "...";
...
const getUserEmails = (users) => {
  const total = [];
  users.map((el) => total.push(el.email));
  return total;
};
...
const getUsersWithAge = (users, minAge, maxAge) => {
  const name = [];
  users.filter((el, ind, arr) =>
    el.age <= maxAge && el.age >= minAge ? name.push(el) : []
  );
  return name;
};
...
const getUsersWithFriend = (users, friendName) => {
  return users
    .filter((el) => el.friends.indexOf(friendName) !== -1)
    .map((el) => el);
};
...
const etotalAveragePlaytimePerGame = players.reduce((total, player) => {
  return total + player.playtime / player.gamesPlayed;
}, 0);
// Дополни функцию getInactiveUsers(users) так, чтобы она возвращала массив неактивных пользователей, значение свойства isActive которых false.
